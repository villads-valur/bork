...
- `~/.config/bork/config.toml` — global config layer (agents allowlist, default_agent, default_prompt, review_prompt, auto_import_reviews, auto_import_authored_prs, etc.). Same flat schema as `<project>/.bork/config.toml`; project values override global. Scalar keys can be read/written with `bork config get|set|list`.
